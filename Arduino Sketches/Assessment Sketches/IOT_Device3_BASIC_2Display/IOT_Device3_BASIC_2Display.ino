...
const uint64_t DIGITS[] = { //Digits with a degree symbol 0=99
...
void displayImage(LedControl& display, uint64_t image, bool rotate = false) {
  for (int i = 0; i < 8; i++) {
...
void handleCommand(String command) {
    ...
    if (isOK || isCaution || isWarning) { // If temperature is present
        if (isStable || isTilted || isTipped) { // If both temperature and position are present
            // Combined logic (as previously defined)
            if (isOK && isStable) {
                lightGreen();
...
void lightRed() {
    ...
    tone(BUZZER, 100, 500); // Warning tone
}

void displayM0Image() {
    for (int i = 0; i < M0_Len; i++) {
        displayImage(display2, M0[i], true);
        delay(500);  // Adjust delay to control speed of animation
    }
}

void displayM1Image() {
    for (int i = 0; i < M1_LEN; i++) {
        displayImage(display2, M1[i], true);
        delay(500);  // Adjust delay to control speed of animation
    }
}
...
void displayM2Image() {
    for (int i = 0; i < M2_LEN; i++) {
        displayImage(display2, M2[i], true);
        delay(500);  // Adjust delay to control speed of animation
    }
}

void displayM3Image() {
    for (int i = 0; i < M3_LEN; i++) {
        displayImage(display2, M3[i], true);
        delay(500);  // Adjust delay to control speed of animation
    }
}
